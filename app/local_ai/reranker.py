...
class NewsReranker:
    def rerank(
        ...
        requested_entities = _expanded_aliases(
            [normalize_text(str(item)) for item in [*context.get("entities", []), *context.get("stock_symbols", [])] if str(item).strip()]
        )
        ...
        for result in results:
            ...
            if combined_entity_score >= 0.75:
                topic_penalty = min(topic_penalty, 0.15)
            ...
            score_breakdown = {
                ...
                "penalty": diversity_penalty + topic_penalty + stock_single_name_penalty + peripheral_entity_penalty,
                "reason": _rerank_reason(combined_entity_score, combined_topic_score, keyword_score, recency_score, topic_penalty),
            }
            updated = dict(result)
            updated.update(
                {
...
def _expanded_aliases(values: list[str]) -> set[str]:
    ...
    for value in values:
        ...
        if compact:
            aliases.add(compact)
        aliases.update(ENTITY_ALIASES.get(normalized, ()))
        aliases.update(ENTITY_ALIASES.get(compact, ()))
    return aliases


def _rerank_reason(entity_score: float, topic_score: float, lexical_score: float, recency_score: float, penalty: float) -> str:
    if penalty > 0.2:
        return "topic_mismatch_penalty_applied"
    if entity_score >= 0.75:
        return "strong_entity_match"
    if topic_score >= 0.75 and lexical_score >= 0.2:
        return "topic_and_lexical_match"
    if topic_score >= 0.75:
        return "topic_match"
    if lexical_score >= 0.3:
        return "lexical_match"
    if recency_score >= 0.5:
        return "recency_match"
