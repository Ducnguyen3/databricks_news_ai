...
def filter_context_for_topic(query: str, topic: str, chunks: list[dict[str, Any]]) -> TopicFilteredContext:
    ...
    for index, chunk in enumerate(chunks):
        ...
        if deny_terms and not _has_any(query_norm, deny_terms):
            ...
            keep_reason = _soft_keep_reason(query_norm, normalized_topic, chunk_topic, text_norm)
            if keep_reason:
                updated = dict(chunk)
                updated["topic_guard_keep_reason"] = keep_reason
                kept.append(updated)
...
def _allow_cross_topic(query_norm: str, topic: str, chunk_topic: str, text_norm: str) -> bool:
    ...
    if topic == "economy_finance_stock" and chunk_topic == "business_startup":
        return _has_any(query_norm, _STOCK_QUERY_TERMS) or _has_any(text_norm, _profile_terms("economy_finance_stock", "allow"))
    if topic == "tech_ai_internet" and chunk_topic == "business_startup":
        return _has_any(text_norm, _profile_terms("tech_ai_internet", "allow"))
    if topic == "world_geopolitics" and chunk_topic == "politics_society":
        return _has_any(text_norm, _profile_terms("world_geopolitics", "allow"))
    return False


def _soft_keep_reason(query_norm: str, topic: str, chunk_topic: str, text_norm: str) -> str:
    if _has_query_entity_overlap(query_norm, text_norm):
        return "kept_by_entity_match_despite_topic_mismatch"
    query_terms = [term for term in query_norm.split() if len(term) >= 3]
    matched_terms = [term for term in query_terms if term in text_norm]
    if len(matched_terms) >= 2:
        return "kept_by_lexical_match_despite_topic_mismatch"
    if _allow_cross_topic(query_norm, topic, chunk_topic, text_norm):
        return "kept_by_allowed_cross_topic_overlap"
    return ""


def _has_query_entity_overlap(query_norm: str, text_norm: str) -> bool:
    return any(term in query_norm and term in text_norm for term in _ENTITY_LIKE_TERMS)
