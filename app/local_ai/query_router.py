...
_MEDIA_ENTITY_KEYWORDS: dict[str, tuple[str, ...]] = {
    "Trung Đông": ("trung dong", "middle east"),
    "Israel": ("israel",),
    "Iran": ("iran",),
    "Gaza": ("gaza",),
...
def route_query(query: str) -> dict[str, Any]:
    ...
    entities = _dedupe_values([*entities, *_media_entities_from_query(normalized)])
    ...
    image_limit = _image_limit_from_query(normalized) if need_images else None
    ...
    route = {
        ...
        "image_limit": image_limit if image_limit is not None else 0,
...
def _image_limit_from_query(normalized: str) -> int:
    patterns = (
        r"(?P<count>\d{1,2})\s*(?:anh|hinh|hinh anh|photo|image|picture)",
        r"(?:anh|hinh|hinh anh|photo|image|picture)\s*(?P<count>\d{1,2})",
    )
...
def _media_entities_from_query(normalized: str) -> list[str]:
    output: list[str] = []
    for entity, aliases in _MEDIA_ENTITY_KEYWORDS.items():
        if any(_keyword_matches(normalized, alias) for alias in aliases):
...
def _has_image_intent(normalized: str) -> bool:
    if "anh huong" in normalized:
        return False
    contextual_hinh_patterns = (
        r"(?<!\w)\d{1,2}\s+hinh(?!\w)",
        r"(?<!\w)(?:cho toi|xem|co|cac|nhung)\s+[^.?!]{0,30}hinh(?!\w)",
        r"(?<!\w)hinh\s+(?:ve|lien quan|nao)(?!\w)",
    )
    if any(re.search(pattern, normalized) for pattern in contextual_hinh_patterns):
        return True
...
def _is_stock_market_overview(normalized: str) -> bool:
    overview_patterns = (
        ...
        "vn-index tuan nay",
        "vnindex tuan nay",
        "thi truong chung khoan tuan nay",
        "chung khoan tuan nay",
    )
    if any(pattern in normalized for pattern in overview_patterns):
        return True
    has_stock_subject = any(term in normalized for term in ("co phieu", "chung khoan", "vn-index", "vnindex"))
    has_market_period = any(term in normalized for term in ("hom nay", "tuan nay", "tuan qua", "7 ngay", "gan day", "thi truong"))
    has_market_question = any(term in normalized for term in ("dien bien", "the nao", "ra sao", "tang giam", "xu huong"))
    if has_stock_subject and has_market_period and has_market_question:
        return True
    return bool(
...
def _source_from_query(normalized: str) -> str | None:
    aliases = {
        "vnexpress": ("vnexpress", "vn express", "vnepress", "vnepxress", "vnexress", "vnexpess"),
        "cafef": ("cafef", "cafe f", "cafef.vn"),
        "genk": ("genk", "gen k", "genk.vn"),
        "diendandoanhnghiep": (
            "diendandoanhnghiep",
