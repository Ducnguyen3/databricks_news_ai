...
class HybridSearchEngine:
    def __init__(
        ...
        bm25_source_count: int | None = None,
    ) -> None:
        self._vector_store = vector_store
        self._embedding_model = embedding_model
        self._bm25_index = bm25_index
        self._bm25_source_count = bm25_source_count
    ...
    def search(self, query: str, query_plan: dict[str, Any], top_n: int = 50) -> list[dict[str, Any]]:
        ...
        self._refresh_bm25_index_if_stale()
        ...
        lexical_results = self._lexical_candidates(query_plan, top_n=top_n)
        return merge_candidates(vector_results, [*bm25_results, *lexical_results], query=query, query_plan=query_plan)

    def _refresh_bm25_index_if_stale(self) -> None:
        get_all_chunks = getattr(self._vector_store, "get_all_chunks", None)
        count = getattr(self._vector_store, "count", None)
        if get_all_chunks is None or count is None:
            return
        try:
            current_count = int(count())
        except Exception:
            return
        if self._bm25_source_count == current_count:
            return
        try:
            chunks = get_all_chunks()
        except Exception:
            return
        self._bm25_index = BM25ChunkIndex(chunks)
        self._bm25_source_count = len(chunks)

    def _lexical_candidates(self, query_plan: dict[str, Any], top_n: int) -> list[dict[str, Any]]:
        if not query_plan.get("requires_lexical"):
            return []
        terms = _lexical_candidate_terms(query_plan)
        if not terms:
            return []
        get_all_chunks = getattr(self._vector_store, "get_all_chunks", None)
        if get_all_chunks is None:
            return []
        try:
            chunks = get_all_chunks()
        except Exception:
            return []
        filters = filters_from_query_plan(query_plan)
        scored: list[dict[str, Any]] = []
        for chunk in chunks:
            metadata = dict(chunk.get("metadata") or {})
            if not _metadata_matches_filters(metadata, filters):
                continue
            score = _lexical_match_score(chunk, terms)
            if score <= 0:
                continue
            scored.append(
                {
        ...
        scored.sort(key=lambda item: safe_float(item.get("bm25_score")), reverse=True)
...
def _lexical_candidate_terms(query_plan: dict[str, Any]) -> list[str]:
    raw_terms = [
        *query_plan.get("stock_symbols", []),
        *query_plan.get("lexical_terms", []),
        *query_plan.get("entities", []),
    ]
    output: list[str] = []
    seen: set[str] = set()
    for term in raw_terms:
        normalized = strip_accents(str(term or "")).lower().strip()
...
def _lexical_match_score(chunk: dict[str, Any], terms: list[str]) -> float:
    metadata = dict(chunk.get("metadata") or {})
    title_entities = strip_accents(
        " ".join(
            [
                str(metadata.get("title") or ""),
                str(metadata.get("entity_names") or ""),
                str(metadata.get("entities_json") or ""),
            ]
        )
    ).lower()
    document = strip_accents(str(chunk.get("document") or chunk.get("text") or "")).lower()
    score = 0.0
    for term in terms:
        if term in title_entities:
            score += 3.0
        if term in document:
            score += 1.0
    return score
